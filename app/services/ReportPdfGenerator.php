...
class ReportPdfGenerator extends TCPDF {
	...
	public function Footer() 
	{
		...
		if ($this->getNumPages() == 1) {
			...
			$confidential_text = 'This report has been prepared solely for use by ABC Ltd. No liability is accepted for any third parties who act upon its contents. No action should be taken without first seeking further professional advice.';
			$this->MultiCell(0, 5, $confidential_text, 0, 'L', 0, 0, '', 265, true);
		} 
		else {
			$img = ($this->CurOrientation == 'P' ? 'head-bg-blue.jpg' : 'head-bg-l.jpg');
			$w = ($this->CurOrientation == 'P ' ? 215.9 : 279.4);
			...
			$text = '(ABC Ltd) REMUNERATION STRATEGIES – TAX SAVING REPORT';
			$this->MultiCell(0, 5, $text, 0, 'L', 0, 0, '', 267, true);
			$this->MultiCell(0, 5, 'Page ' . $this->getNumPages(), 0, 'R', 0, 0, '', 267, true);
		}
	...
	public function buildCoverPage()
	{
		...
		$this->MultiCell(190, 5, '(ABC Ltd)', 0, 'L', 0, 0, '', 65, true);
